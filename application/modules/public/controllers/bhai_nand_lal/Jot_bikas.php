...
class Jot_bikas extends My_Controller
{
    function Jot_Bikas()
    {
        parent::__construct();
    ...
    function index($index = 1)
    {
        ...
        if ($index == (int)1)
        $index = (get_cookie('jot_bikas_page_no') != false ? get_cookie('jot_bikas_page_no') : 1);

        $lines = $this->bhai_nand_lal_dao->get_jot_bikas_lines($index);
        ...
        if (get_last_segment() == "pdf_view") {
            ...
            $html = $this->load->view('forms/page-by-page/jot-bikas', $page, true);
        ...
        $this->load->view('forms/page-by-page/jot-bikas', $page);
    }
    ...
    function page($index = 1, $d = 'line', $line_no = 'NA')
    {
	    ...
	    if($d == 'ajax'){
            echo json_encode([
                'content' => $this->load->viewPartial('forms/page-by-page/jot-bikas', $page),
    ...
    function shabad($shabad_id = 0, $line = 0, $lineno = 0){
        ...
        $page['meta_keywords']    = 'Bhai Nand Lal - jot-bikas shabad , bnl-shabad,';
        $page['meta_title'] = 'Bhai Nand Lal - jot-bikas - SearchGurbani.com';
        $page['meta_description'] = 'Bhai Nand Lal - jot-bikas - SearchGurbani.com';
        ...
        $this->load->view('forms/page-by-page/bhai-nand-lal-jot-bikas-shabad', $page);
    ...
    function verse($verse_id,$d = null)
    {
        ...
        $page['base_url'] = $SG_BNL['jot_bikas']['controller_name_dash'] . '/verse';
        $page['lines'] = $this->common_dao->get_line_verse($data);
        $page['data'] = $page['lines']->result_array();
        $page['start_page'] = 1231;
        $page['end_page'] = 1321;
        $page['current_page'] = $verse_id;
        // load the page
        $page['theme'] = 'theme_7';
        $page['meta_title'] = 'Bhai Nand Lal - Jot Bikas Verse';
        ...
        if($d == 'ajax'){
            echo json_encode([
                'content' => $this->load->viewPartial('forms/page-by-page/bhai-nand-lal-jot-bikas-verse', $page),
                'title' => $page['meta_title'],
                'description' => $page['meta_description'],
                'keywords' => $page['meta_keywords'],
            ]);
        }else{
            $this->load->view('forms/page-by-page/bhai-nand-lal-jot-bikas-verse', $page);
    ...
    function search($text = null, $pageNo = 0)
    {
        ...
        $searchcount['total'] = $this->bhai_nand_lal_dao->searchCountJot_Bikas($text);

        $totalRows = count($searchcount['total']);
        //echo $totalRows;exit;

        $this->load->library('pagination');
        $config = array('base_url' => site_url('bhai_nand_lal/jot_bikas/search/' . $text),
            'total_rows' => $totalRows,
            'per_page' => '10'
        );
        $this->pagination->initialize($config);
    
        $page['lines'] = $this->bhai_nand_lal_dao->searchTextJot_Bikas($text, $config['per_page'], $pageNo);
        ...
        $this->load->view('forms/page-by-page/searchjot-bikas', $page);
    }
    ...
    function ajax_remember_me()
    {
        ...
        set_cookie('jot_bikas_page_no', $page_no, 365 * 24 * 60 * 60);
    }
}