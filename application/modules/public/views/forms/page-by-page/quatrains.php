   ...
   <div class="row">
		<div class="col-md-6 col-sm-6">
			...
			<form name="goto_page" id="goto_page" action="" method="post" onsubmit="return gotoPage(this);">
				Goto Ang <input type="text" name="page_no" id="page_no" class="form-control" style="width: 15%" size="5" maxlength="5" value="' . $current_page . '" />
...
$navigationBar .= '
	...
	<div class="row">
		...
		<div class="col-md-4 col-xs-4 center">
			<div class="ang-count bold">Displaying Rubaayee ' . $current_page . ' <span> of ' . $SG_BNL[$scripture]['page_to'] . '</span></div>
		</div>
		<div class="col-md-4 col-xs-4"></div>	
	</div>	
</div>';
...
echo '<h3 class="no-top">Bhai Nand Lal - Rubaayee</h3>';

echo $utilityBar;
echo $navigationBar;

$printable_languages = get_printable_languages('quatrains');
...
if ($lines === false) {
    ...
    if ($SG_Preferences['text_type'] == 'line_by_line') {
        ...
        foreach ($lines->result() as $line) {
            ...
            $sharing_data = '<span class="share_icons"><a class="btn btn-default shabad_view" href="' . site_url('bhai-nand-lal/quatrains/shabad/' . $line->shabadID . '/line/' . $line->shabadlineID) . '">Shabad View</a>';

            $sharing_data .= '<a class="btn btn-default verse_view" href="' . site_url('bhai-nand-lal/quatrains/verse/' . $line->verseID) . '"> Verse View</a>';
            ...
            $share_data['link'] = site_url('public/shared/bhai-nand-lal/quatrains/page/' . $current_page . '/line/' . $line->lineno);
            $share_data['whatsapp_sharelink'] = site_url('public/bhai-nand-lal/quatrains/page/' . $current_page . '/line/' . $line->lineno);
