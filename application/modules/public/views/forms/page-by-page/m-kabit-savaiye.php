	...
	<span class="col_33_percent align_right col_float_right"> 
		<span class="navigation_links">';
            ...
            if ($current_page > $SG_ScriptureTypes[$scripture]['page_from']) {
                $navigationBar .= '<a href="' . site_url('scriptures/kabit/' . $SG_ScriptureTypes[$scripture]['page_from']) . '" class="button">Begin</a> ';
                if ($current_page + 1 > $SG_ScriptureTypes[$scripture]['page_from']) {
                    $navigationBar .= '<a href="' . site_url('scriptures/kabit/' . ($current_page - 1)) . '" class="button">Back</a> ';
                }
            }

            if ($current_page < $SG_ScriptureTypes[$scripture]['page_to']) {
                $navigationBar .= '<a href="' . site_url('scriptures/kabit/' . ($current_page + 1)) . '" class="button">Next</a> ';
                if ($current_page - 1 < $SG_ScriptureTypes[$scripture]['page_to']) {
                    $navigationBar .= '<a href="' . site_url('scriptures/kabit/' . $SG_ScriptureTypes[$scripture]['page_to']) . '" class="button">Last</a> ';
                }
            }
            $navigationBar .= '
	...
	';
                ...
                if ($SG_Preferences['text_type'] == 'line_by_line') {
                    ...
                    foreach ($lines->result() as $line) {
                        ...
                        $share_data['whatsapp_sharelink'] = site_url('kabit_savaiye/kabit/' . $current_page . '/line/' . $line->lineno);
            ...
            <script type="text/javascript">
                function gotoPage(formobj) {
                    ...
                    document.location.href = '<?php echo site_url('scriptures/kabit'); ?>/' + page_no;
                    return false;
                }
