...
echo '<h3 class="no-top">Bhai Nand Lal - Ganjnama</h3>';

echo $utilityBar;

echo $navigationBar;

$printable_languages = get_printable_languages('ganjnama');
...
if ($lines === false) {
    ...
    if ($SG_Preferences['text_type'] == 'line_by_line') {
        ...
        foreach ($lines->result() as $line) {
            ...
            $sharing_data = '<span class="share_icons"><a class="btn btn-default shabad_view" href="' . site_url('bhai-nand-lal/ganjnama/shabad/' . $line->shabadID . '/line/' . $line->shabadlineID) . '">Shabad View</a>';
            ...
            $share_data['link'] = site_url('shared/bhai-nand-lal/ganjnama/page/' . $current_page . '/line/' . $line->lineno);
            $share_data['whatsapp_sharelink'] = site_url('bhai-nand-lal/ganjnama/page/' . $current_page . '/line/' . $line->lineno);
