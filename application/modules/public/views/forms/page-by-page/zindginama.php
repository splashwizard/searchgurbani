...
echo '<h3 class="no-top">Bhai Nand Lal - Zindginama</h3>';

echo $utilityBar;
echo $navigationBar;

$printable_languages = get_printable_languages('zindginama');
...
if ($lines === false) {
    ...
    if ($SG_Preferences['text_type'] == 'line_by_line') {
        ...
        foreach ($lines->result() as $line) {
            ...
            $sharing_data = '<span class="share_icons"><a class="btn btn-default shabad_view" href="' . site_url('bhai-nand-lal/zindginama/shabad/' . $line->shabadID . '/line/' . $line->shabadlineID) . '">Shabad View</a>';

            $sharing_data .= '<a class="btn btn-default verse_view" href="' . site_url('bhai-nand-lal/zindginama/verse/' . $line->verseID) . '"> Verse View</a>';
            ...
            $share_data['link'] = site_url('public/shared/bhai-nand-lal/zindginama/page/' . $current_page . '/line/' . $line->lineno);
            $share_data['whatsapp_sharelink'] = site_url('public/bhai-nand-lal/zindginama/page/' . $current_page . '/line/' . $line->lineno);
