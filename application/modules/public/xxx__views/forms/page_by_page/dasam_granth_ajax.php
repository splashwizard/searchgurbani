	...
	';
		...
		if ($SG_Preferences['text_type'] == 'line_by_line') {
			...
			foreach ($lines->result() as $line) {
				...
				$attributes = '<p class="line_info">' . $line->lineno . '' . $line->pagelineno . ' ' . $line->lattrib . '</p>';
				
				$sharing_data = '<span class="share_icons">';
				$share_data['text'] = $line->punjabi . ' - ' . $line->translit;
				$share_data['link'] = site_url('public/shared/dasam-granth/page/' . $current_page . '/line/' . $line->pagelineno);
				...
				{
					echo get_data_by_preferences_lareevar($lareevar_line,'lareevar');
				}
				
				if(!empty($Lareevar_assist))
				{
					echo get_data_by_preferences_lareevar($Lareevar_assist,'lareevar_assist_button');
				}
