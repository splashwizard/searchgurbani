...
<script type="text/javascript">
    function search_pageR(formobj1) {
        ...
        document.location.href = '<?php echo site_url($SG_BNL[$scripture]['controller_name_dash'] . '/search'); ?>/' + serachText;
        return false;
    }

    <?php /*?>function gotoPage(formobj)
{


	var page_no = formobj.page_no.value;
	document.location.href='<?php echo site_url($SG_BNL[$scripture]['controller_name_dash'].'/page'); ?>/' + page_no;
	return false;
}
<?php */?>
    $('.remember_this').click(function () {
        $.ajax({
            url: '<?php echo site_url($SG_BNL[$scripture]['controller_name_dash'] . '/ajax-remember-me/' . $current_page); ?>',
