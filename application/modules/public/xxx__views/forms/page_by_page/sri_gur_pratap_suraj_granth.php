...
if ($this->session->userdata('book_lang') == '' or $this->session->userdata('book_lang') == 'gurmukhi') {
    $lang_button = '<a href="' . site_url('sri-gur-pratap-suraj-granth/page/' . $page_no . '/volume/' . $volume_id . '/hindi') . '" class="button">View in Hindi</a>';
    $display_lang_field = 'text';
} else {
    $lang_button = '<a href="' . site_url('sri-gur-pratap-suraj-granth/page/' . $page_no . '/volume/' . $volume_id . '/gurmukhi') . '" class="button">View in Gurmukhi</a>';
...
<div class="utility_bar">
	...
	<span class="col_float_left" style="padding-top: 4px;">
		<div class="displaying_page_info">
			Displaying Page ' . $page_no . ' of ' . $lines_count . ' from Volume ' . $volume_id . '
		</div>
	</span>
	<span class="align_right col_float_right"> 
		<span class="navigation_links">
			' . $lang_button . '
			<a href="' . site_url('sri-gur-pratap-suraj-granth/') . '" class="button">Search Page</a> 
			<a href="' . site_url('sri-gur-pratap-suraj-granth/chapters/' . $volume_id) . '" class="button">Chapter Index</a> ';

if ($page_no > 1) {
    $navigationBar .= '<a href="' . site_url('sri-gur-pratap-suraj-granth/page/1/volume/' . $volume_id) . '" class="button">Begin</a> ';
    if ($page_no + 1 > 1) {
        $navigationBar .= '<a href="' . site_url('sri-gur-pratap-suraj-granth/page/' . ($page_no - 1)) . '/volume/' . $volume_id . '" class="button">Back</a> ';
    }
}

if ($page_no < $lines_count) {
    $navigationBar .= '<a href="' . site_url('sri-gur-pratap-suraj-granth/page/' . ($page_no + 1)) . '/volume/' . $volume_id . '" class="button">Next</a> ';
    if ($page_no - 1 < $lines_count) {
        $navigationBar .= '<a href="' . site_url('sri-gur-pratap-suraj-granth/page/' . $lines_count . '/volume/' . $volume_id) . '" class="button">Last</a> ';
    }
}
$navigationBar .= '
...
<script type="text/javascript">
    function gotoPage(formobj) {
        ...
        document.location.href = '<?php echo site_url('sri-gur-pratap-suraj-granth/page'); ?>/' + page_no + '<?php echo '/volume/' . $volume_id; ?>';
        return false;
    }
