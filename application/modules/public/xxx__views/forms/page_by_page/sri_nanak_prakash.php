...
if ($this->session->userdata('book_lang') == '' or $this->session->userdata('book_lang') == 'gurmukhi') {
    $lang_button = '<a href="' . site_url('sri-nanak-prakash/page/' . $page_no . '/hindi') . '" class="button">View in Hindi</a>';
    $display_lang_field = 'text';
} else {
    $lang_button = '<a href="' . site_url('sri-nanak-prakash/page/' . $page_no . '/gurmukhi') . '" class="button">View in Gurmukhi</a>';
...
<div class="utility_bar">
	...
	<span class="align_right col_float_right"> 
		<span class="navigation_links">
			' . $lang_button . '
			<a href="' . site_url('sri-nanak-prakash/') . '" class="button">Search Page</a> 
			<a href="' . site_url('sri-nanak-prakash/chapters/' . $volume_id) . '" class="button">Chapter Index</a> ';

if ($page_no > 1) {
    $navigationBar .= '<a href="' . site_url('sri-nanak-prakash/page/1') . '" class="button">Begin</a> ';
    if ($page_no + 1 > 1) {
        $navigationBar .= '<a href="' . site_url('sri-nanak-prakash/page/' . ($page_no - 1)) . '" class="button">Back</a> ';
    }
}

if ($page_no < $lines_count) {
    $navigationBar .= '<a href="' . site_url('sri-nanak-prakash/page/' . ($page_no + 1)) . '" class="button">Next</a> ';
    if ($page_no - 1 < $lines_count) {
        $navigationBar .= '<a href="' . site_url('sri-nanak-prakash/page/' . $lines_count) . '" class="button">Last</a> ';
    }
}
$navigationBar .= '
...
<script type="text/javascript">
    function gotoPage(formobj) {
        ...
        document.location.href = '<?php echo site_url('sri-nanak-prakash/page'); ?>/' + page_no;
        return false;
    }
